...
const KIJIJI_BASE_URL = proxy + 'https://kijiji.ca';
...
function getPageResults(params, pageNum, firstResultPagePath) {
    ...
    const pagePath = firstResultPagePath.replace(LOCATION_REGEX, `$1/$2`);

    // Search Kijiji
    console.log(proxy2 + pagePath)
    return fetch(proxy2 + pagePath)
        .then(res => res.text())
        .then(body => {
